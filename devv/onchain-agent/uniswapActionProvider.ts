 ...
 */
export class Uniswapv2ActionProvider extends ActionProvider<WalletProvider> {
  /**
  ...
  async swapEthToUsdc(
    ...
    if (!this.supportsNetwork(network)) {
      throw new Error(`This action is only supported on Base Sepolia network. Current network: ${network.networkId}`);
    }
    ...
    try {
      ...
      // Format ETH amount with full precision for small values
      const formattedEthAmount = args.ethAmount < 0.0001 
        ? args.ethAmount.toFixed(8) 
        : args.ethAmount.toString();

      return `
        Swap completed successfully!
        Transaction hash: ${txHash}
        ETH amount: ${formattedEthAmount} ETH
      `;
    } catch (error: unknown) {
  ...
  supportsNetwork(network: Network): boolean {
    // Only support Base Sepolia testnet and EVM protocol family
    return network.protocolFamily === EVM_PROTOCOL_FAMILY && 
