 ...
 *
 * 1. Select your LLM:
 *    - Modify the `ChatGroq` instantiation to 
...
export async function createAgent(): Promise<ReturnType<typeof createReactAgent>> {
  ...
  if (!process.env.GROQ_API_KEY) {
    throw new Error("I need a GROQ_API_KEY in your .env file to power my intelligence.");
  }

  const { agentkit, walletProvider } = await prepareAgentkitAndWalletProvider();

  try {
    // Initialize LLM: https://console.groq.com/docs/models
    ...
    agent = createReactAgent({
      ...
      messageModifier: `
        ...
        you can use usiswapActionProvider to swap ETH to usdc on the uniswap v2 protocol.
        IMPORTANT: If you encounter function calling limitations, explain the situation to the user and suggest manual alternatives.`,
    });
